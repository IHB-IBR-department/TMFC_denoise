% Example of TMFC_denoise usage #1: Block design
%
% Auditory fMRI dataset from the SPM website: 
...
data_path = fullfile(data_path,'auditory');
...
for iSub = 1:nSub
    SPM_paths{iSub,1} = fullfile(data_path,['sub-' sprintf('%02d', iSub)],'GLM','SPM.mat');  % Paths to SPM.mat files
...
% Option 2 - GUI:
% Select the 'sM00223' folder and apply the 'sM*.img' text filter
% anat_paths = tmfc_select_anat_GUI(subject_paths);

%% Select realigned and unsmoothed functional images in MNI space
%--------------------------------------------------------------------------
% Option 1 - Command line:
func_subfolder = 'fM00223';
for iSub = 1:length(SPM_paths)
    func_files = spm_select('ExtList', fullfile(data_path,['sub-' sprintf('%02d', iSub)],func_subfolder), '^wfM.*\.img$');
...
% Option 2 - GUI:
% Select the fM00223 folder and apply the ^wfM*.img text filter
