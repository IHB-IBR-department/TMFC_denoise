...
``TMFC_denoise`` first checks whether existing SPM segmentation outputs or previously created ``TMFC_denoise`` masks are available. 
If found, these files are reused to avoid unnecessary recomputation.
...
Masks are computed as follows:
 ...
 1) Segmentation of the T1-weighted structural image in native space. This step produces tissue-probability maps used to create individual binary masks (Ashburner & Friston, 2005). Before running segmentation, ``TMFC_denoise`` searches for existing SPM segmentation outputs, including GM, WM, and CSF tissue-probability maps (``c1``, ``c2``, ``c3``) and the deformation field (``y_``). If these files are found, they are reused for mask generation. If they are not found, ``TMFC_denoise`` performs segmentation and stores the outputs in the ``TMFC_denoise/Segmentation`` subfolders. These outputs can be reused to generate binary masks with different parameters without repeating segmentation.
 ...
 7) Calculation of the final masks in MNI space. The eroded WM and CSF masks, the GM mask, and the skull-stripped structural image are normalized to MNI space using the deformation field estimated during segmentation. They are then resliced to match the voxel grid of the first functional image. No additional coregistration transform is estimated at this stage. In the next step, brainstem voxels are removed from the WM mask by subtracting the binary brainstem mask from the Harvard-Oxford atlas (Desikan et al., 2006; following Mascali et al., 2021). The CSF mask is restricted to the ventricles by multiplying it with the binary ventricular mask from the Automatic Lateral Ventricle delIneatioN (ALVIN) atlas (Kempton et al., 2011; following Muschelli et al., 2014 and Mascali et al., 2021). Finally, all binary masks are multiplied by the implicit SPM mask generated during model estimation (mask.nii), which includes voxels with sufficient BOLD signal intensity.
